class CensusRecordsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @census_record.save
        format.html { redirect_to new_census_record_path, notice: 'Census record was successfully created.' }
        format.json { render action: 'show', status: :created, location: @census_record }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to census_records_url }
      format.json { head :no_content }
    end
  end
